...
# ── Scheduled AWS cost sync ─────────────────────────────────────────────────
# Fires every 3 days, hits POST /api/ops/v1/finance/sources/aws/sync.
# Cost: ~$0.30/mo in CE calls (1 sync per 3 days * 3 CE calls * $0.01).
# Maintainer can still trigger ad-hoc syncs from the admin dashboard
# button — this cron is the "always have current numbers when I open
# the dashboard" baseline.

resource "aws_cloudwatch_event_rule" "lingo_ops_cost_sync" {
  name                = "lingo-ops-cost-sync"
  description         = "Trigger lingo-ops AWS cost sync every 3 days"
  schedule_expression = "rate(3 days)"
  tags                = merge(local.common_tags, { Domain = "ops" })
}

resource "aws_cloudwatch_event_target" "lingo_ops_cost_sync" {
  rule = aws_cloudwatch_event_rule.lingo_ops_cost_sync.name
  arn  = aws_lambda_function.lingo_ops.arn

  # Function URL routes by rawPath; we synthesize the minimum payload
  # Mangum needs to dispatch this as a POST to the sync route.
  input = jsonencode({
    version = "2.0"
    rawPath = "/api/ops/v1/finance/sources/aws/sync"
    requestContext = {
      http = {
        method = "POST"
        path   = "/api/ops/v1/finance/sources/aws/sync"
      }
    }
    headers = {}
  })
...
resource "aws_lambda_permission" "lingo_ops_cost_sync" {
  statement_id  = "AllowEventBridgeInvokeCostSync"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lingo_ops.function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.lingo_ops_cost_sync.arn
}
